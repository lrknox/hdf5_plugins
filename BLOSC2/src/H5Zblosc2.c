...
static size_t blosc2_filter_function(unsigned int flags, size_t cd_nelmts, const unsigned int cd_values[],
...
const H5Z_class2_t blosc2_H5Filter[1] = {{
    ...
    "HDF5 blosc2 filter; see "
    "https://github.com/HDFGroup/hdf5_plugins/blob/master/docs/RegisteredFilterPlugins.md", /* Filter name for
                                                                                               debugging */
...
blosc2_set_local(hid_t dcpl, hid_t type, hid_t space)
{
    ...
    if (1 < ndim && ndim <= B2ND_MAX_DIM) {
        ...
        if (nelements < 6) {
            values[5] = DEFAULT_SHUFFLE;
        }
        if (nelements < 7) {
            values[6] = DEFAULT_COMPCODE;
        }
...
int32_t
compute_blosc2_blocksize(int32_t chunksize, int32_t typesize, int clevel, int compcode)
{
 ...
 */
int32_t
compute_b2nd_block_shape(size_t block_size, size_t type_size, const int rank, const int32_t *dims_chunk,
                         int32_t *dims_block)
{
    assert(block_size >= 0);
    assert(type_size >= 0);
    ...
    if (nitems_new > nitems) {
        BLOSC_TRACE_INFO("Target block size is too small (%lu items), raising to %lu items", nitems,
                         nitems_new);
    }
    if (nitems_new >= nitems) {
 ...
 */
size_t
blosc2_filter_function(unsigned int flags, size_t cd_nelmts, const unsigned int cd_values[], size_t nbytes,
                       size_t *buf_size, void **buf)
{
    ...
    if (!(flags & H5Z_FLAG_REVERSE)) {
        ...
        if (cd_nelmts >= 7) {
            ...
            if (code == -1) {
                sprintf(errmsg,
                        "this Blosc2 library does not have support for the '%s' compressor, but only for: %s",
                        compname, complist);
                PUSH_ERR("blosc2_filter", H5E_CALLBACK, errmsg);
                goto failed;
            }
        ...
        if (ndim > 1) {
            ...
            if (blocksize == 0) {
                int32_t sugg_blocksize = compute_blosc2_blocksize(outbuf_size, typesize, clevel, compcode);
                if (sugg_blocksize < 0) {
            ...
            if (!(ctx = b2nd_create_ctx(&storage, ndim, chunkshape_l, chunkshape, blockdims, dtype,
        ...
        if (blosc2_meta_exists(schunk, "b2nd") >= 0 || blosc2_meta_exists(schunk, "caterva") >= 0) {
            ...
            for (int i = 0; i < array->ndim; i++) {
                ...
                if (ndim >= 0 && array->shape[i] != chunkshape[i]) {
                     ...
                     */
                    sprintf(errmsg, "B2ND array shape[%d] (%ld) != filter chunkshape[%d] (%d)", i,
                            array->shape[i], i, chunkshape[i]);
                    PUSH_ERR("blosc2_filter", H5E_CALLBACK, errmsg);
                    goto b2nd_decomp_out;
                }
            }
            ...
            if (b2nd_get_slice_cbuffer(array, start, stop, outbuf, stop, outbuf_size) < 0) {
